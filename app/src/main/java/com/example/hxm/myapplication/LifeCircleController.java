...
public class LifeCircleController implements Pluginable {
    ...
    /**
     * 加载插件Activity * * @param classLoader * @param pluginName * @return
     */
    ...
    public void onCreate(Bundle extras) {
        ...
        mPluginApk = getPluginApk(packageName);
//         获取加载的插件APK信息
        assert mPluginApk != null;
        mPluginActivity = loadPluginActivity(mPluginApk.classLoader, pluginName);
        // 加载插件Activity
        mPluginActivity.attach(mActivityProxy);
        // 绑定代理Activity到插件Activity中, 使其能够调用代理Activity对应的应用资源等方法
        mPluginActivity.onCreate(null); // 代理生命周期


    }

    private PluginApk getPluginApk(String packageName) {
        return null;
    }
